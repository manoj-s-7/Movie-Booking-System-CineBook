...
const FORMAT_STYLE = {
  "2D": "bg-[#1a1a1a] border-[rgba(139,92,246,0.22)] text-[#b3b3b3]",
...
export default function MovieDetailPage() {
  ...
  useEffect(() => {
    ...
    moviesAPI.showtimes(id, selectedDate)
      ...
      .catch(() => { setShowtimes([]); setShowtimeError(true); })
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-12 py-8">
        <div className="grid lg:grid-cols-3 gap-8 mb-12">
          ...
          {/* Info card */}
          <div className="bg-[#141414] rounded-xl border border-[rgba(139,92,246,0.10)] p-5">
            {[
              ["Status", movie.status],
              ...
              <div key={k} className="flex justify-between py-2.5 border-b border-[rgba(139,92,246,0.08)] last:border-0">
        ...
        <div>
              ...
              <button key={iso} onClick={() => setSelectedDate(iso)}
                className={`flex-none flex flex-col items-center w-16 py-3 rounded-xl border transition-all ${
                  selectedDate === iso
                    ...
                    : "bg-[#141414] border-[rgba(139,92,246,0.13)] text-[#b3b3b3] hover:border-[rgba(139,92,246,0.28)] hover:text-white"
                }`}>
          ...
          {loadingST ? (
            ...
            <div className="bg-[#141414] border border-red-500/20 rounded-xl p-8 text-center">
              <Ticket size={28} className="text-red-500/40 mx-auto mb-3" />
              <p className="text-white font-semibold mb-1">Could not load showtimes</p>
              <p className="text-[#555] text-sm mb-4">Make sure the backend is running and DB is connected</p>
              <button
                onClick={() => { setLoadingST(true); setShowtimeError(false); moviesAPI.showtimes(id, selectedDate).then(({ data }) => setShowtimes(Array.isArray(data) ? data : [])).catch(() => { setShowtimes([]); setShowtimeError(true); }).finally(() => setLoadingST(false)); }}
                className="btn-red px-5 py-2 rounded-lg text-sm mx-auto flex items-center gap-2"
              >
                Retry
              </button>
            </div>
          ) : Object.keys(byTheater).length === 0 ? (
            <div className="bg-[#141414] border border-[rgba(139,92,246,0.10)] rounded-xl p-8 text-center">
              <Ticket size={28} className="text-[#333] mx-auto mb-3" />
              <p className="text-white font-semibold mb-1">No shows yet for this date</p>
              <p className="text-[#555] text-sm">Try selecting a different date or check back later</p>
            </div>
          ) : (
            <div className="space-y-3">
              {Object.entries(byTheater).map(([theater, { location, shows }]) => (
                <div key={theater} className="bg-[#141414] border border-[rgba(139,92,246,0.10)] rounded-xl p-5 hover:border-[rgba(139,92,246,0.20)] transition-colors">
