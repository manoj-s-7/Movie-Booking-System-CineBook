...
import { Alert, Tooltip, LinearProgress } from "@mui/material";
import toast from "react-hot-toast";

// Standard FIRST, Premium SECOND, Recliner THIRD
const SEAT_TYPE_ORDER = { standard: 0, premium: 1, recliner: 2 };

const SEAT_TYPE_CONFIG = {
  standard: { label: "Standard",  color: "#94a3b8", bg: "rgba(148,163,184,0.1)",  border: "rgba(148,163,184,0.3)",  multiplier: 1.0 },
  premium:  { label: "Premium",   color: "#f59e0b", bg: "rgba(245,158,11,0.1)",   border: "rgba(245,197,24,0.4)",   multiplier: 1.3 },
  recliner: { label: "Recliner",  color: "#a855f7", bg: "rgba(168,85,247,0.1)",   border: "rgba(168,85,247,0.4)",   multiplier: 1.8 },
};
...
function SeatLegend() {
  return (
    <div className="flex justify-center gap-4 flex-wrap text-xs mb-4">
      {[
        { label: "Available", style: { background: "rgba(255,255,255,0.05)", border: "1.5px solid rgba(255,255,255,0.12)", borderRadius: "4px" } },
        { label: "Selected",  style: { background: "#e50914", border: "1.5px solid #ff4757", borderRadius: "4px" }, textColor: "text-red-400" },
        { label: "Booked",    style: { background: "rgba(255,255,255,0.02)", border: "1.5px solid rgba(255,255,255,0.04)", borderRadius: "4px", opacity: 0.4 } },
        { label: "Premium",   style: { background: "rgba(245,158,11,0.1)", border: "1.5px solid rgba(245,197,24,0.4)", borderRadius: "4px" }, textColor: "text-yellow-400" },
        { label: "Recliner",  style: { background: "rgba(168,85,247,0.1)", border: "1.5px solid rgba(168,85,247,0.4)", borderRadius: "7px" }, textColor: "text-purple-400" },
      ].map(({ label, style, textColor = "text-[#555]" }) => (
...
export default function BookingPage() {
  ...
  const toggleSeat = (seat) => {
    ...
    setSelectedSeats(prev => {
      ...
      if (prev.length >= MAX_SEATS) { toast.error(`Maximum ${MAX_SEATS} seats per booking`); return prev; }
      return [...prev, seat];
    });
  };
  ...
  // Rows sorted: Standard rows first, then Premium, then Recliner
  const rows = useMemo(() => {
    return [...new Set(seats.map(s => s.row_label))].sort((a, b) => {
      ...
      return (SEAT_TYPE_ORDER[typeA] - SEAT_TYPE_ORDER[typeB]) || a.localeCompare(b);
    });
  }, [seats]);
  ...
  // Group rows by seat type in correct order
      ...
      <div className="pt-20 max-w-7xl mx-auto px-4 sm:px-6 py-6">
          ...
          {poster ? (
            <div className="flex-none">
              <Image
                ...
                alt={title || "Movie"}
                width={60} height={90}
                className="rounded-xl object-cover border border-purple-500/20 shadow-2xl"
              />
            </div>
          ) : (
            <div className="w-[60px] h-[90px] rounded-xl bg-white/5 border border-purple-500/18 flex items-center justify-center flex-none">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              ...
              <div className="text-right hidden sm:block">
                <p className="text-[#444] text-xs mb-1">Starting from</p>
                <p className="text-2xl font-black text-white">₹{price.toFixed(0)}</p>
                <p className="text-[#444] text-xs">per seat</p>
              </div>
            </div>
          </div>
        </div>
          ...
          {[["1","Select Seats",true],["2","Payment",false],["3","Confirmed",false]].map(([num,label,active],i) => (
            <div key={num} className="flex items-center gap-1.5">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all ${
                active ? "bg-red-600 border-red-500 text-white shadow-lg shadow-red-500/30" : "border-purple-500/18 bg-white/4 text-[#333]"
              }`}>{num}</div>
              <span className={`text-sm hidden sm:block font-medium ${active ? "text-white" : "text-[#2a2a2a]"}`}>{label}</span>
              {i < 2 && <ChevronRight size={13} className="text-[#1e1e1e]" />}
            </div>
          ))}
        </div>
        ...
        <div className="grid xl:grid-cols-3 gap-5">
          ...
          <div className="xl:col-span-2">
            ...
            {!loading && (
              <Alert severity="info" className="mb-4" sx={{ borderRadius: "12px" }}>
                <strong>Seat order (screen to back):</strong> Standard → Premium → Recliner
              </Alert>
            )}
              ...
              <div className="flex items-center gap-1.5">
                <Tooltip title="Zoom out" arrow>
                  <button
                    ...
                    className="w-8 h-8 rounded-lg bg-white/5 border border-purple-500/15 flex items-center justify-center text-[#555] hover:text-white transition-all hover:bg-white/10"
                  ><ZoomOut size={14} /></button>
                </Tooltip>
                <button
                  onClick={() => setZoom(1)}
                  className="w-8 h-8 rounded-lg bg-white/5 border border-purple-500/15 flex items-center justify-center text-[#555] hover:text-white transition-all hover:bg-white/10 text-xs font-mono"
                >
                  {Math.round(zoom * 100)}%
                </button>
            ...
            <div className="text-center mb-6">
              <div className="relative mx-auto max-w-sm">
                ...
                <div className="h-0.5 rounded-full mx-4 opacity-25" style={{ background: "linear-gradient(to right,transparent,#ff6b8a,transparent)" }} />
                <p className="text-[9px] text-[#333] mt-2 tracking-[0.4em] uppercase">All eyes this way · Screen</p>
              </div>
            </div>
            ...
            {loading ? (
              ...
              <div className="bg-[#111] rounded-2xl p-6 overflow-x-auto" style={{ border: "1px solid rgba(255,255,255,0.05)" }}>
                <div className="min-w-max mx-auto transition-all duration-200" style={{ transform: `scale(${zoom})`, transformOrigin: "top center" }}>
                  {rowGroups.map(({ type, rows: groupRows }) => (
                    <div key={type} className="mb-4">
                      {/* Section header */}
                      <div className="flex items-center gap-3 mb-2.5">
                        <div className="h-px flex-1 opacity-25" style={{ background: SEAT_TYPE_CONFIG[type]?.color }} />
                        <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg" style={{ background: `${SEAT_TYPE_CONFIG[type]?.bg}`, border: `1px solid ${SEAT_TYPE_CONFIG[type]?.border}` }}>
                          ...
                          <span className="text-[9px]" style={{ color: SEAT_TYPE_CONFIG[type]?.color, opacity: 0.6 }}>
                            · ₹{(price * SEAT_TYPE_CONFIG[type]?.multiplier).toFixed(0)}/seat
                          </span>
                        </div>
                        <div className="h-px flex-1 opacity-25" style={{ background: SEAT_TYPE_CONFIG[type]?.color }} />
                      </div>
                      ...
                      {groupRows.map(rowLabel => {
                        ...
                        return (
                          <div key={rowLabel} className="flex items-center gap-2 mb-1.5">
                            <span className="w-5 text-[10px] text-[#2a2a2a] font-mono text-right flex-none">{rowLabel}</span>
                            <div className="flex gap-1.5">
                              {rowSeats.map((seat, seatIdx) => (
                                <div key={seat.id} className="flex items-center gap-1.5">
                                  {seatIdx === Math.floor(rowSeats.length / 2) && <div className="w-5" />}
                                  <Tooltip
            ...
            <div className="glass rounded-2xl p-5 glow-border sticky top-24 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-white flex items-center gap-2">
                  <Ticket size={16} className="text-red-500" /> Your Selection
                </h3>
                {selectedSeats.length > 0 && (
                  <span className="text-xs text-[#444] font-mono bg-white/4 px-2 py-1 rounded-lg">
              ...
              {selectedSeats.length === 0 ? (
                <div className="text-center py-10 border border-dashed border-[rgba(139,92,246,0.10)] rounded-xl">
                  <div className="w-14 h-14 rounded-full bg-white/4 flex items-center justify-center mx-auto mb-3">
                    <Ticket size={24} className="text-[#2a2a2a]" />
                  </div>
                  <p className="text-[#444] text-sm font-medium">No seats selected</p>
                  <p className="text-[#2a2a2a] text-xs mt-1">Click a seat on the map</p>
                </div>
              ) : (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="w-6 h-6 rounded-lg bg-red-500/8 hover:bg-red-500/18 flex items-center justify-center transition-all"
                          >
              ...
              {/* Pricing */}
              <div className="border-t border-[rgba(139,92,246,0.10)] pt-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-[#555]">Subtotal ({selectedSeats.length} seat{selectedSeats.length !== 1 ? "s" : ""})</span>
                  <span className="text-white font-mono">₹{totalAmount.toFixed(0)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#555] flex items-center gap-1">Convenience Fee <span className="text-[#2a2a2a] text-[10px]">(5%)</span></span>
                  <span className="text-white font-mono">₹{convenience.toFixed(0)}</span>
                </div>
                <div className="flex justify-between font-bold text-base pt-2 border-t border-[rgba(139,92,246,0.10)]">
              ...
              >
                {selectedSeats.length === 0 ? "Select Seats First" : <>Proceed to Payment <ChevronRight size={18} /></>}
              </button>

              {selectedSeats.length > 0 && (
                <button
                  onClick={() => setSelectedSeats([])}
                  className="w-full flex items-center justify-center gap-1.5 py-2 text-xs text-[#333] hover:text-[#555] transition-colors"
                >
                  <RotateCcw size={11} /> Clear selection
                </button>
              )}

              <p className="text-center text-[#2a2a2a] text-[10px]">
