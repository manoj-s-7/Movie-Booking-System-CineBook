...
const cancelBooking = async (req, res) => {
  ...
  try {
    ...
    // Free up seats
    // MySQL JSON columns may already be parsed objects; guard against double-parsing
    const seatsRaw = booking[0].seats;
    const seats = typeof seatsRaw === "string" ? JSON.parse(seatsRaw) : seatsRaw;
    const seatNums = Array.isArray(seats)
      ? seats.map((s) => (typeof s === "object" ? s.seat_number : s)).filter(Boolean)
      : [];
    await pool.query(
